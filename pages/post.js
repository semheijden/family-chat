...
export default function Post() {
    ...
    const submitPost = async (e) => {
        ...
        if(post?.hasOwnProperty('id')){
            ...
            setPost({description: ''})
            toast.success('Post has been made 🚀', {position: toast.POSITION.TOP_LEFT, autoClose: 1500})
            return route.push('/')
        }
    ...
    return (
        <div className='my-20 p-12 shadow-lg rounded-lg max-w-md mx-auto'>
            <form onSubmit={submitPost}>
                ...
                <button type='submit' className='w-full bg-cyan-600 text-white font-medium my-2 p-2 rounded-lg'>{post.hasOwnProperty('id') ? "Edit" : "Submit"}</button>
            </form>
        </div>
    )
}